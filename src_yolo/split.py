...
def show_segmented_characters_cv2(characters):
    """
    使用 OpenCV 显示分割后的字符图像
    :param characters: 分割后的字符图像集 (列表，每个元素是 NumPy 数组)
    """
    if not characters or len(characters) == 0:
        print("No characters to display!")
        return

    # 创建一个黑色背景，宽度为所有字符宽度之和，高度为字符的最大高度
    ...
    for char in characters:
        ...
        combined_image[0:height, current_x:current_x + width] = char
        current_x += width

    # 显示拼接后的图像
    cv2.imshow("Segmented Characters", combined_image)
...
def split_char(image):
    ...
    #  显示分割后的字符
    show_segmented_characters_cv2(segments)
