...
def process_single_image(image_path, model, output_dir, writer):
    ...
    result = results.pred[0]  # 获取预测结果

    if len(result) > 0:  # 检测到车牌
        box = result[0, :4].cpu().numpy().astype(int)  # 置信度最高的车牌
        x1, y1, x2, y2 = box
...
train_output_csv = "CCPD/cropped/train_labels.csv"
val_output_csv = "CCPD/cropped/val_labels.csv"
